...
// Nic8e D7ay!" the output should be 2. First if you add up all the numbers, 6 + 9 + 2 + 8 + 7 you get 32. Then there are 17
// letters in the string. 32 / 17 = 1.882, and the final answer should be rounded to the nearest whole number, so the answer is 2.
...
function NumberSearch(str) {
  let nums = str.match(/\d/gm);
  let letters = str.match(/[a-zA-Z]/gm);
  return nums && letters.length
    ? Math.round(
        nums.reduce((a, c) => parseInt(a) + parseInt(c)) / letters.length
      )
    : 0;
}

console.log(NumberSearch('YYYYYYYYY')); // 0
