...
// arr is [3, 1, 3, 5, 10, 6, 4, 3, 1] then your program should output "1,2,3,5,6,6,4,3"
...
function MovingMedian(arr) {
  const window = arr.shift();
  let resArr = [];
  for (let i = 1; i <= arr.length; i++) {
    let start = i - window <= 0 ? 0 : i - window;
    resArr.push(median(arr.slice(start, i)));
  }
  return resArr.join(',');
}

console.log(MovingMedian([5, 2, 4, 6])); //Output: 2,3,4
console.log(MovingMedian([3, 0, 0, -2, 0, 2, 0, -2])); //Output: 0, 0, 0, 0, 0, 0, 0;
console.log(MovingMedian([3, 1, 3, 5, 10, 6, 4, 3, 1])); //Output: 1,2,3,5,6,6,4,3;
